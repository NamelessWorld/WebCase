/*!
* [Wed Apr 25 2018 11:51:04 GMT+0800 (CST)]
*/
/*!
 *
...
(function (JsMatrix, undefined) {
    ...
    JsMatrix.prototype.extend({
        ...
        // 把新的矩阵合并到现在矩阵的左边
        'leftPend': function (jsMatrix) {

            jsMatrix = JsMatrix(jsMatrix);
            if (jsMatrix.row == this.row) {
                var row, col;
                for (row = 0; row < this.row; row++) {
                    for (col = 0; col < this.col; col++) {
                        jsMatrix.value[row][col + jsMatrix.col] = this.value[row][col];
                    }
                }

            } else {
                throw new Error('leftPend:The newly added matrix needs the same number of rows as the original matrix!');
            }

            if (this.isDeterminant) {
                return new JsMatrix(jsMatrix);
            } else {
                this.col += jsMatrix.col;
                this.value = jsMatrix.value;
                return this;
            }
        },

        // 把新的矩阵合并到现在矩阵的右边
        'rightPend': function (jsMatrix) {
            jsMatrix = JsMatrix(jsMatrix);
            var tempArray = [];
            if (jsMatrix.row == this.row) {
                var row, col;
                for (row = 0; row < jsMatrix.row; row++) {
                    tempArray[row] = [];
                    for (col = 0; col < this.col; col++) {
                        tempArray[row][col] = this.value[row][col];
                    }
                    for (col = 0; col < jsMatrix.col; col++) {
                        tempArray[row][col + this.col] = jsMatrix.value[row][col];
                    }
                }
            } else {
                throw new Error('rightPend:The newly added matrix needs the same number of rows as the original matrix!');
            }
            if (this.isDeterminant) {
                return new JsMatrix(tempArray);
            } else {
                this.col += jsMatrix.col;
                this.value = jsMatrix.value;
                return this;
        ...
        // 把新的矩阵合并到现在矩阵的上边
        'topPend': function (jsMatrix) {
            jsMatrix = JsMatrix(jsMatrix);
            if (jsMatrix.col == this.col) {
                var row, col;
                for (row = 0; row < this.row; row++) {
                    jsMatrix.value[row + jsMatrix.row] = [];
                    for (col = 0; col < this.col; col++) {
                        jsMatrix.value[row + jsMatrix.row][col] = this.value[row][col];
                    }
                }
            } else {
                throw new Error('topPend:The newly added matrix needs to be the same as the column number of the original matrix!');
            }
            if (this.isDeterminant) {
                return new JsMatrix(jsMatrix);
            } else {
                this.row += jsMatrix.row;
                this.value = jsMatrix.value;
                return this;
            }
        },
        ...
        'bottomPend': function (jsMatrix) {
            ...
            if (jsMatrix.col == this.col) {
                ...
                for (row = 0; row < jsMatrix.row; row++) {
                    tempArray[row + this.row] = [];
                    for (col = 0; col < jsMatrix.col; col++) {
                        tempArray[row + this.row][col] = jsMatrix.value[row][col];
                    }
                }
            } else {
                throw new Error('bottomPend:The newly added matrix needs to be the same as the column number of the original matrix!');
            }
            if (this.isDeterminant) {
                return new JsMatrix(tempArray);
            } else {
                this.col += jsMatrix.row;
...
/*!======= auxiliary.replace.js =======*/
(function (JsMatrix, undefined) {

    // 扩展JsMatrix对象上的方法
    JsMatrix.prototype.extend({

        // 用新的矩阵替换旧矩阵的某些行[index：开始替换的起点，isStrict：需要替换的元素是否需要严格存在]
        'rowReplace': function (jsMatrix, index, isStrict) {
            jsMatrix = JsMatrix(jsMatrix);
            if ('number' == typeof index) {
                if (index != index.toFixed(0)) {
                    throw new Error('rowReplace:The second parameter must be an integer!');
                }
            } else {
                throw new Error('rowReplace:The second parameter must be a number!');
            }
            if (jsMatrix.col == this.col) {
                if (index >= 0 && index < this.row) {
                    if (isStrict || this.isDeterminant) {
                        if (index + jsMatrix.row - 1 >= this.row) {
                            throw new Error('rowReplace:End the coordinate cross boundary[strict]!');
                        }
                        this.row = index + jsMatrix.row;
                    }
                    var row, col;
                    for (row = 0; row < jsMatrix.row; row++) {
                        this.value[row + index] = [];
                        for (col = 0; col < jsMatrix.col; col++) {
                            this.value[row + index][col] = jsMatrix.value[row][col];
                        }
                    }
                } else {
                    throw new Error('rowReplace:Start the coordinate cross boundary!');
                }
            } else {
                throw new Error('rowReplace:The replacement matrix needs to be the same as the column number of the original matrix!');
            }
            return this;
        },

        // 用新的矩阵替换旧矩阵的某些列[index：开始替换的起点，isStrict：需要替换的元素是否需要严格存在]
        'colReplace': function (jsMatrix, index, isStrict) {
            jsMatrix = JsMatrix(jsMatrix);
            if ('number' == typeof index) {
                if (index != index.toFixed(0)) {
                    throw new Error('colReplace:The second parameter must be an integer!');
                }
            } else {
                throw new Error('colReplace:The second parameter must be a number!');
            }
            if (jsMatrix.row == this.row) {
                if (index >= 0 && index < this.col) {
                    if (isStrict || this.isDeterminant) {
                        if (index + jsMatrix.col - 1 >= this.col) {
                            throw new Error('colReplace:End the coordinate cross boundary[strict]!');
                        }
                        this.col = index + jsMatrix.col;
                    }
                    var row, col;
                    for (row = 0; row < jsMatrix.row; row++) {
                        for (col = 0; col < jsMatrix.col; col++) {
                            this.value[row][col + index] = jsMatrix.value[row][col];
                        }
                    }
                } else {
                    throw new Error('colReplace:Start the coordinate cross boundary!');
                }
            } else {
                throw new Error('colReplace:The replacement matrix needs the same number of rows as the original matrix!');
            }
            return this;
        }
...
(function (JsMatrix, undefined) {
    ...
    JsMatrix.prototype.extend({
        ...
        // 非零乘矩阵中某一行
        '__transform_row_1__': function (row, num) {
            if ('number' != typeof row || 'number' != typeof num) {
                throw new Error('__transform_row_1__:The parameter should be two numbers!');
            }
            if (num == 0) {
                throw new Error('__transform_row_1__:The second parameter can not be zero!');
            }
            if (row != row.toFixed(0)) {
                throw new Error('__transform_row_1__:The first parameter must be an integer!');
            }
            if (row < 0 || row >= this.row) {
                throw new Error('__transform_row_1__:Transboundary error!');
            }
            var col;
            for (col = 0; col < this.col; col++) {
                this.value[row][col] *= num;
            }
            return this;
        },

        // 某一行C倍加到另一行
        '__transform_row_2__': function (fromRow, toRow, num) {
            if ('number' != typeof fromRow || 'number' != typeof toRow || 'number' != typeof num) {
                throw new Error('__transform_row_2__:The parameter should be two numbers!');
            }
            if (num == 0) {
                throw new Error('__transform_row_2__:The third parameter can not be zero!');
            }
            if (fromRow != fromRow.toFixed(0) || toRow != toRow.toFixed(0)) {
                throw new Error('__transform_row_2__:The first two parameters need to be integers!');
            }
            if (fromRow < 0 || fromRow >= this.row || toRow < 0 || toRow >= this.row) {
                throw new Error('__transform_row_2__:Transboundary error!');
            }
            var col;
            for (col = 0; col < this.col; col++) {
                this.value[toRow][col] += num * this.value[fromRow][col];
            }
            return this;
        },

        // 互换二行位置
        '__transform_row_3__': function (row1, row2) {
            if ('number' != typeof row1 || 'number' != typeof row2) {
                throw new Error('__transform_row_3__:The parameter should be two numbers!');
            }
            if (row1 != row1.toFixed(0) || row2 != row2.toFixed(0)) {
                throw new Error('__transform_row_3__:The first two parameters need to be integers!');
            }
            if (row1 < 0 || row1 >= this.row || row2 < 0 || row2 >= this.row) {
                throw new Error('__transform_row_3__:Transboundary error!');
            }
            var col;
            for (col = 0; col < this.col; col++) {
                this.value[row1][col] += this.value[row2][col];
                this.value[row2][col] = this.value[row1][col] - this.value[row2][col];
                this.value[row1][col] = this.value[row1][col] - this.value[row2][col];
            }
            return this;
        },

        // 非零乘矩阵中某一列
        '__transform_col_1__': function (col, num) {
            if ('number' != typeof col || 'number' != typeof num) {
                throw new Error('__transform_col_1__:The parameter should be two numbers!');
            }
            if (num == 0) {
                throw new Error('__transform_col_1__:The second parameter can not be zero!');
            }
            if (col != col.toFixed(0)) {
                throw new Error('__transform_col_1__:The first parameter must be an integer!');
            }
            if (col < 0 || col >= this.col) {
                throw new Error('__transform_col_1__:Transboundary error!');
            }
            var row;
            for (row = 0; row < this.row; row++) {
                this.value[row][col] *= num;
        ...
        // 某一列C倍加到另一列
        '__transform_col_2__': function (fromCol, toCol, num) {
            if ('number' != typeof fromCol || 'number' != typeof toCol || 'number' != typeof num) {
                throw new Error('__transform_col_2__:The parameter should be two numbers!');
            }
            if (num == 0) {
                throw new Error('__transform_col_2__:The third parameter can not be zero!');
            }
            if (fromCol != fromCol.toFixed(0) || toCol != toCol.toFixed(0)) {
                throw new Error('__transform_col_2__:The first two parameters need to be integers!');
            }
            if (fromCol < 0 || fromCol >= this.col || toCol < 0 || toCol >= this.col) {
                throw new Error('__transform_col_2__:Transboundary error!');
            }
            var row;
            for (row = 0; row < this.row; row++) {
                this.value[row][toCol] += num * this.value[row][fromCol];
            }
            return this;
        },

        // 互换二列位置
        '__transform_col_3__': function (col1, col2) {
            if ('number' != typeof col1 || 'number' != typeof col2) {
                throw new Error('__transform_col_3__:The parameter should be two numbers!');
            }
            if (col1 != col1.toFixed(0) || col2 != col2.toFixed(0)) {
                throw new Error('__transform_col_3__:The first two parameters need to be integers!');
            }
            if (col1 < 0 || col1 >= this.col || col2 < 0 || col2 >= this.col) {
                throw new Error('__transform_col_3__:Transboundary error!');
            }
            var row;
            for (row = 0; row < this.row; row++) {
                this.value[row][col1] += this.value[row][col2];
                this.value[row][col2] = this.value[row][col1] - this.value[row][col2];
                this.value[row][col1] = this.value[row][col1] - this.value[row][col2];
            }
            return this;
        }
